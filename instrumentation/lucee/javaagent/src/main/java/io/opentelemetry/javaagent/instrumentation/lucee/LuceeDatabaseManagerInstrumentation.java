...
public class LuceeDatabaseManagerInstrumentation implements TypeInstrumentation {
  ...
  public Map<? extends ElementMatcher<? super MethodDescription>, String> transformers() {
    return singletonMap(
        isMethod().and(takesArguments(0))
            .and(named("begin").or(named("rollback")).or(named("savepoint")).or(named("commit")).or(named("end"))),
        LuceeDatabaseManagerInstrumentation.class.getName() + "$CfDatabaseManager");
  }

  public static class CfDatabaseManager {
    @Advice.OnMethodEnter(suppress = Throwable.class)
    public static void methodEnter(@Advice.Origin("#m") String methodName, @Advice.Local("otelContext") Context context,
